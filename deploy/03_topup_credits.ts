...
const deployContracts: DeployFunction = async function (
  ...
  const artifact = await hre.artifacts.readArtifact(TopupCreditContractName);
  ...
  console.log(
    `The topupCredit is deployed at ${await topupCredit.getAddress()}`
  );


  const fullContractSource = `${artifact.sourceName}:${artifact.contractName}`;

  const address = await topupCredit.getAddress();
...
//   await deployContract(TopupCreditContractName, []);
// }
